...
const data = "./Scrapping/old_data.json"
...
function getTotalHrefs() {
    ...
    return jsonData.map(item => {
        ...
        const formattedDate = `${day}/${monthToNumber(month)}/${year}`;
        return [item.href, formattedDate, item.city];
    });
}
...
async function scrapeFacPage(url, date, ville ,index, totalIndex) {
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        const adresse = getText("#adresse .callout-text.max-w-52w span");
        const allAddressFields = document.querySelectorAll("#adresse .callout-text span");
        const codePostal = allAddressFields.length >= 3 ? allAddressFields[2].innerText.trim() : null;
  
        // Récupération du contact
        const telephone = document.querySelector("#contact .callout-text span")?.innerText.trim().replace(" ", "") || null;
  ...
  async function scrapeAllFacs() {  
    ...
    try {
      fs.writeFileSync("fac_data.json", JSON.stringify(allData, null, 2), "utf-8");
